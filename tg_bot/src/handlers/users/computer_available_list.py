...
async def computer_available_list(call: CallbackQuery, computer_api: ComputerAPI, state: FSMContext):
    ...
    if not computers:
        ...
        return
    await state.update_data(available_computers=computers)

    page = 1
    total_pages = len(computers)
    computer = computers[page - 1]
    message_text, keyboard = generate_computer_message(computer, page, total_pages, page_type="available_computers", add_booking=True)
    await call.message.edit_text(
        text=message_text,
...
async def available_computers_pagination(call: CallbackQuery, callback_data: PageNumber, state: FSMContext):
    ...
    computers = (await state.get_data()).get("available_computers")
    if action == "prev":
        page -= 1
    elif action == "next":
        page += 1
    ...
    computer = computers[page - 1]
    message_text, keyboard = generate_computer_message(computer, page, len(computers), page_type="available_computers", add_booking=True)
